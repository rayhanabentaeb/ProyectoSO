...
void *AtenderCliente(void *socket) {
    ...
    while (terminar == 0) {
       ...
       if (codigo == 0) { // Petición de desconexión
            ...
            if (numForm == 1) { // Se recibe el nombre
                strcpy(nombre, p);
                sprintf(respuesta, "Nombre recibido: %s", nombre);
                write(sock_conn, respuesta, strlen(respuesta));
            } 
            else if (numForm == 2) { // Se recibe el correo
                strcpy(correo, p);
                sprintf(respuesta, "Correo recibido: %s", correo);
                write(sock_conn, respuesta, strlen(respuesta));
            } 
            else if (numForm == 3) { // Se recibe la contraseña y se completa el registro
                strcpy(contrasena, p);
                ...
                sprintf(query, "INSERT INTO Player (Id, Username, Name, Address, Password) VALUES (NULL, '%s', '%s', '%s', '%s')", nombre, nombre, correo, contrasena);
                if (mysql_query(conn, query)) {
                ...
                sprintf(respuesta, "Registro completado");
                write(sock_conn, respuesta, strlen(respuesta));
            }
        }
    }
